...
/* 
  // Since Storage Service endpoint is being enabled, Firewall app rule is not required. 
  // Service endpoint takes precedence over the Firewall route rule (UDR)
  rule {
    name = "uc-metastore-storage"
    ...
    target_fqdns = ["${var.metastore_storage_name}.dfs.core.windows.net"]
